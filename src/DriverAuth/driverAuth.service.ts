...
export class AuthService {
  ...
  constructor(
   ...
   async register(registerDto: RegisterDto): Promise<Omit<Driver, 'password'>> {
    ...
    // Create new driver
    const newDriver = new this.driverModel(userData);
  ...
  async loginWithEmail(loginEmailDto: LoginEmailDto) {
    // Find user by email
    const user = await this.driverModel.findOne({
  ...
  async signUpWithGoogle(googleSignupDto: GoogleSignupDto) {
    try {
      ...
      let user = await this.driverModel.findOne({
      ...
      if (user) {
        ...
        user = new this.driverModel(userData);
        await user.save();
      }
  ...
  async requestPasswordReset(forgetPasswordDto: ForgetPasswordRequestDto): Promise<{ message: string}> {
    ...
    // Find user by email
    const user = await this.driverModel.findOne({ email });
